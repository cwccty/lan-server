...
export const connectionMethodCatalog: ConnectionMethodEntry[] = [
  {
    ...
    shortLabel: '组网',
    status: '已接入',
    summary: '当前主线组网方式。双方进入同一个联机房间后，游戏把联机地址当作局域网地址使用。',
    whenToUse: '原生局域网、IP 直连、专用服务端、需要端口代理或房间发现辅助的游戏。',
    userSteps: ['在加入与组网页保存房间名、密钥和中继地址', '启动组网并等待状态确认', '好友使用邀请包一键加入'],
  ...
  },
  {
    ...
    shortLabel: 'TCP 代理',
    status: '已接入',
    summary: '把一个本地 TCP 监听端口转发到目标联机地址的游戏端口。',
    whenToUse: '游戏服务端只监听本机地址，或需要把特定 TCP 端口开放给好友。',
    userSteps: ['确认游戏实际 TCP 端口', '填写本地监听端口和目标联机地址', '启动后做端口检测'],
  ...
  },
  {
    ...
    shortLabel: 'UDP 代理',
    status: '已接入',
    summary: '把 UDP 单播数据转发到目标联机地址与端口。',
    whenToUse: '游戏使用 UDP 单播连接，但直接连接联机地址不稳定或端口需要转发。',
    userSteps: ['确认游戏 UDP 端口', '填写监听端口、目标联机地址和目标端口', '启动后观察转发状态'],
  ...
  },
  {
    ...
    shortLabel: '广播桥',
    status: '已接入',
    summary: '把局域网大厅发现用的 UDP 广播包转发到联机房间。',
    whenToUse: '游戏支持局域网大厅，但好友看不到房间列表。',
    userSteps: ['先确保组网已连接', '确认游戏大厅发现 UDP 端口', '在高级工具创建广播桥并观察转发状态'],
  ...
  },
  {
    ...
    status: '引导',
    summary: '更标准的组网方式，适合有固定服务器、愿意配置密钥和路由的高级用户。',
    whenToUse: '内置组网不适合、需要更强可控性，或管理员已经有 WireGuard 服务端。',
  ...
  },
  {
    ...
    whenToUse: '用户已经熟悉 ZeroTier，或不想自建中继服务器。',
