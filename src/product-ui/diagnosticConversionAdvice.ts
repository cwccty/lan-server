...
export function buildDiagnosticConversionAdvice(
  ...
  if (route.kind === 'remote_coop') {
    const suggestedActions = [
      '停止把这个问题当作组网或联机地址故障排查；这类游戏不需要好友连接联机地址。',
      '回到推荐方案页，使用 Steam Remote Play Together；非 Steam 或需要更细控制时用 Sunshine + Moonlight。',
      '重点检查串流质量、输入权限、手柄/键鼠映射，而不是中继地址或游戏端口。',
    ];
    return {
      id: 'remote-coop-not-n2n',
      tone: hasN2nSymptoms ? 'warning' : 'info',
      title: '当前游戏更像本地同屏远程联机，不应优先修组网',
      summary: '诊断里即使出现组网或中继地址问题，也不一定影响这类游戏联机；正确路线是远程同屏。',
      routeHint: route.summary,
      whyDiagnosticMatters: '避免用户在只能同屏的游戏上反复配置虚拟局域网，却仍无法在游戏内输入 IP 加入。',
      suggestedActions,
      copyText: buildAdviceCopyText(game, route, assessment, '本地同屏游戏不要优先修组网', suggestedActions),
      targetTab: 'protocol',
  ...
  if (route.kind === 'steam_p2p') {
    const suggestedActions = [
      ...
      '不要把“联机地址端口不通”当成主要问题，除非管理员已经确认存在 Steam Relay / P2P 插件方案。',
      '如需插件化路线，先把评估和证据带到方案库，由管理员确认反作弊、账号和插件边界。',
    ];
    return {
      id: 'steam-p2p-not-virtual-ip',
      tone: hasN2nSymptoms || hasPortSymptoms ? 'warning' : 'info',
      title: '当前游戏优先走 Steam 大厅 / P2P，不应默认连接联机地址',
      summary: 'Steam P2P 游戏的失败通常不靠组网端口检测判断，应该先确认 Steam 原生邀请是否可用。',
      routeHint: route.summary,
      whyDiagnosticMatters: '避免用户看到端口检测失败后误以为组网修好就能加入 Steam 大厅游戏。',
      suggestedActions,
  ...
  if (route.kind === 'official_only') {
    const suggestedActions = [
      '不要继续生成局域网邀请包，也不要让好友连接联机地址。',
      '使用游戏官方服务器、官方大厅或官方账号入口。',
      '把“官方服限定”的原因写入游戏方案，避免其他用户继续尝试错误方案。',
    ];
    return {
      ...
      routeHint: route.summary,
      whyDiagnosticMatters: '官方账号、匹配或后端鉴权无法靠虚拟局域网替代；继续排查组网会浪费时间。',
      suggestedActions,
  ...
  if (route.kind === 'udp_broadcast_bridge') {
    const suggestedActions = [
      '保留组网，但不要只看状态确认；这类游戏可能还需要 UDP 广播桥才能发现局域网大厅。',
      '进入高级连接工具，配置 UDP 广播桥和游戏大厅发现端口。',
      '如果支持手动 IP 直连，优先让好友连接房主联机地址；否则复制诊断报告给管理员补充广播端口证据。',
    ];
    return {
      id: 'udp-broadcast-needs-bridge',
      tone: hasBroadcastSymptoms || hasPortSymptoms ? 'warning' : 'info',
      title: '当前路线需要 UDP 广播桥，单独组网可能只能互通但看不到房间',
      summary: '局域网大厅发现类游戏需要把“组网成功”和“房间被发现”分开排查。',
      routeHint: route.summary,
      whyDiagnosticMatters: '联机确认正常只证明联机房间互通，不证明游戏局域网广播已经被转发。',
      suggestedActions,
  ...
  if (route.kind === 'tcp_port_proxy') {
    const suggestedActions = [
      '保留组网，同时确认游戏端口监听地址、TCP/UDP 协议和目标联机地址。',
    ...
    return {
      ...
      summary: '这类游戏不是只启动组网就一定可连，还要确认端口已经对好友开放。',
      routeHint: route.summary,
  ...
  if (route.kind === 'needs_review') {
    ...
    return {
      ...
      summary: '缺少可靠游戏方案时，诊断很难判断该修组网、广播桥、端口代理还是远程同屏。',
      routeHint: route.summary,
      whyDiagnosticMatters: '先确认游戏原始多人能力，才能避免“按钮很多但方向错误”。',
      suggestedActions,
      copyText: buildAdviceCopyText(game, route, assessment, '缺少游戏方案时先复核游戏类型', suggestedActions),
      targetTab: 'solutions',
      shouldShowAdvancedTools: false,
      shouldCreateAdapterIntent: true,
      shouldDeprioritizeN2nFixes: false,
