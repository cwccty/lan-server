...
export const connectionCapabilityMatrix: ConnectionCapabilityDecisionRow[] = [
  {
    ...
    userFacingResult: '优先推荐 n2n 虚拟局域网，好友用房主虚拟 IP 和端口加入。',
    ...
    adapterDefaults: {
      ...
      defaultNotes: [
        '游戏支持 LAN 或 IP 直连；优先使用 n2n 虚拟局域网。',
        '好友加入同一房间后，在游戏内连接房主虚拟 IP 和端口。',
      ],
    },
  ...
  },
  {
    ...
    verdictLabel: '可转局域网',
    recommendedMethodIds: ['n2n', 'wireguard', 'zerotier', 'tailscale'],
    userFacingResult: '先组网，再启动服务端，好友连接房主虚拟 IP 和服务端端口。',
    ...
    adapterDefaults: {
      ...
      defaultNotes: [
        ...
        '好友连接房主虚拟 IP 和服务端端口；服务端窗口关闭会导致房间不可用。',
      ],
    },
  ...
  },
  {
    ...
    adapterDefaults: {
      ...
      defaultNotes: [
        '游戏只能本地同屏，不能被转换成真正 LAN；不要生成虚拟 IP 加入说明。',
        '推荐 Steam Remote Play Together，备用 Sunshine + Moonlight；重点检查串流质量和输入权限。',
      ],
    },
    operatorDecision: '保存 adapter 时必须写清“不要连接虚拟 IP”，避免误导用户。',
    riskNote: '远程同屏依赖串流质量；它解决输入和画面共享，不会把游戏改成真正 LAN。',
  },
  {
    id: 'steam-p2p-lobby',
    gameType: 'Steam 大厅 / Steam P2P',
    capability: '游戏没有 LAN 入口，但有 Steam 好友邀请、大厅或 P2P 联机。',
    routeKind: 'steam_p2p',
    verdict: 'can_connect_not_lan',
    verdictLabel: '可联机，但不默认转 LAN',
    ...
    adapterDefaults: {
      ...
      canConvertToLan: false,
      defaultNotes: [
        '游戏主要依赖 Steam 大厅或 Steam P2P；默认不推荐强行 n2n 转 LAN。',
        '优先保留 Steam 原生邀请；Steam Relay / P2P 插件只作为人工确认后的未来入口。',
      ],
    },
    operatorDecision: '不要默认推荐 n2n；需要管理员确认插件边界和用户风险。',
  ...
  },
  {
    ...
    evidenceToCollect: ['官方服务器/账号依赖说明', '是否无直连/无 LAN/无本地同屏', '转换失败证据'],
    adapterFields: ['multiplayer_conversion.can_convert_to_lan: false', 'notes', 'troubleshooting'],
    adapterDefaults: {
      networkType: 'official_only',
      canConvertToLan: false,
      defaultNotes: [
        '游戏多人依赖官方服务器、账号或匹配后端；不建议转换成 LAN。',
        '向用户解释限制，保留官方入口；不要生成 n2n 邀请包或虚拟 IP 连接步骤。',
      ],
    },
  ...
  },
  {
    id: 'unknown-review',
    gameType: '未知 / 待人工确认',
    capability: '扫描到了游戏，但缺少 adapter 或多人能力证据不足。',
    routeKind: 'needs_review',
    verdict: 'needs_review',
    verdictLabel: '先复核',
    recommendedMethodIds: [],
    userFacingResult: '先同步共享库；若仍缺失，由管理员确认类型后保存 adapter。',
...
export function buildConnectionCapabilityMatrixGuide(rows = connectionCapabilityMatrix) {
  return [
    ...
    '',
    '原则：目标不是强行把所有游戏变成 LAN，而是识别游戏原本多人能力，再选择最合适的联机方案。',
    '',
    ...rows.map((row, index) => [
      ...
      `   adapter 信号：${row.adapterSignals.join('；')}`,
      `   需要证据：${row.evidenceToCollect.join('；')}`,
