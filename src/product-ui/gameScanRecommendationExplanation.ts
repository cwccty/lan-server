...
function routeReason(kind: AdapterRouteKind) {
  const map: Record<AdapterRouteKind, { headline: string; reason: string; nextStep: string; actionLabel: string }> = {
    virtual_lan: {
      headline: '可按虚拟局域网开房',
      reason: 'adapter 判断该游戏支持 LAN 或 IP 直连，组好 n2n 后让好友连接房主虚拟 IP 和端口。',
      nextStep: '进入推荐方案页，按开房向导启动 n2n、检测端口并复制邀请包。',
      actionLabel: '进入开房向导',
    },
    dedicated_server: {
      headline: '需要先启动服务端',
      reason: 'adapter 判断该游戏需要房主启动专用服务端或内置开服程序，再通过虚拟 IP 加入。',
      nextStep: '进入推荐方案页，先启动组网，再启动服务端并检测端口。',
      actionLabel: '进入服务端向导',
    },
    udp_broadcast_bridge: {
      headline: '需要 n2n + UDP 广播桥',
      reason: 'adapter 判断该游戏依赖局域网大厅广播发现，单纯 n2n 可能无法让大厅互相发现。',
      nextStep: '进入推荐方案页，按向导启动 n2n，再到高级工具配置 UDP 广播桥。',
      actionLabel: '查看桥接方案',
    },
    tcp_port_proxy: {
      headline: '需要 n2n + 端口代理',
      reason: 'adapter 判断该游戏需要把本机监听端口代理到虚拟网，单纯启动 n2n 可能还不够。',
      nextStep: '进入推荐方案页，按向导启动 n2n，再到高级工具配置端口代理。',
      actionLabel: '查看代理方案',
    },
    remote_coop: {
      headline: '推荐远程同屏联机',
      reason: 'adapter 判断该游戏主要是本地同屏/本地合作，不应该强行转换为 LAN。',
      nextStep: '进入推荐方案页，复制 Steam Remote Play 或 Sunshine + Moonlight 的好友说明。',
      actionLabel: '查看远程同屏方案',
    },
    steam_p2p: {
      headline: '优先使用 Steam 大厅/P2P',
      reason: 'adapter 判断该游戏依赖 Steam 大厅、好友邀请或 P2P 流程，不默认把 n2n 当主方案。',
      nextStep: '进入推荐方案页查看 Steam 路线说明；如果未来接入 Steam Relay 插件，可在这里扩展。',
      actionLabel: '查看 Steam 方案',
    },
    official_only: {
      headline: '不建议强转局域网',
      reason: 'adapter 判断该游戏更适合保留官方服务器、官方大厅或官方账号流程。',
      nextStep: '查看限制说明，避免用户花时间配置 n2n 后仍无法联机。',
      actionLabel: '查看限制说明',
    },
    needs_review: {
      headline: '需要管理员复核',
      reason: '当前 adapter 信息不足，尚不能可靠判断它属于 LAN、服务端、广播桥、远程同屏、Steam P2P 还是官方服限定。',
      nextStep: '先去方案库同步共享库或创建自建 adapter，再回到推荐页执行。',
      actionLabel: '去方案库复核',
    },
  };
  return map[kind];
