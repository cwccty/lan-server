...
## 26. 产品级记忆：通用组网优先（2026-06-02 补充）

本项目从现在开始按“通用组网中心 + 可选游戏向导”的产品结构推进。

核心原则：

- n2n / Radmin / Manual LAN 是通用网络能力，不属于 Terraria 专属配置。
- 游戏向导只负责特定游戏的开服、参数、端口、邀请文本和诊断；不重复实现组网。
- UI 状态必须由真实运行状态驱动，不能做显示层掩盖。
- 自检结果必须能复现，且失败时能定位到进程、端口、虚拟 IP、supernode、TAP、防火墙或游戏服务端具体环节。
