...
const DEFAULT_CONNECTTOOL_DIR: &str = "E:\\BaiduNetdiskDownload\\connecttool-qt-1.5.7\\connecttool-qt-windows-x86_64";
...
pub fn get_steam_relay_status_for_dir(connecttool_dir: Option<String>) -> SteamRelayStatus {
    ...
    if next_steps.is_empty() && connecttool_status.can_start {
        next_steps.push("点击“启动 ConnectTool helper”，再在 helper 里创建房间或加入房间。".to_string());
    }
...
fn build_native_status(steam: &(bool, Option<String>)) -> SteamRelayNativeStatus {
    ...
    let precheck_ready = steam.0 && sdk_dir.is_some() && redistributable_path.is_some() && app_id.is_some();
    SteamRelayNativeStatus {
        built_in: false,
        available: false,
        state: if precheck_ready { "precheck_ready_not_built_in" } else { "not_built_in" }.to_string(),
        message: if precheck_ready {
            "原生 Steamworks 依赖预检基本满足，但联机助手尚未内置真实 Steamworks Networking。".to_string()
        } else {
...
fn native_status_reasons(native_status: &SteamRelayNativeStatus, steam: &(bool, Option<String>)) -> Vec<String> {
    let mut reasons = Vec::new();
    if !steam.0 {
        reasons.push("未检测到正在运行的 Steam 客户端。".to_string());
    }
    if !native_status.built_in {
        reasons.push("联机助手尚未内置原生 Steamworks P2P；请使用 ConnectTool 兼容模式。".to_string());
    }
    reasons
}
...
pub fn start_connecttool_helper(input: ConnectToolLaunchRequest) -> Result<SteamRelayStatus, String> {
    ...
    let dir = status.connecttool_status.directory.clone().ok_or_else(|| "ConnectTool 目录为空".to_string())?;
    ...
    *connecttool_child_store().lock().map_err(|_| "ConnectTool process lock poisoned".to_string())? = Some(child);
...
pub fn stop_connecttool_helper(input: ConnectToolLaunchRequest) -> Result<SteamRelayStatus, String> {
    ...
    if let Some(mut child) = connecttool_child_store().lock().map_err(|_| "ConnectTool process lock poisoned".to_string())?.take() {
        let _ = child.kill();
        let _ = child.wait();
    } else {
...
pub fn start_steam_p2p_host(input: SteamP2pHostRequest) -> Result<SteamP2pSessionStatus, String> {
    ...
    let app_id = input.app_id.or_else(|| status.connecttool_status.app_id.clone()).or_else(|| status.app_id.clone()).unwrap_or_default();
    ...
    let session = SteamP2pSessionStatus {
        ...
        state: if status.connecttool_status.helper_running { "helper_running" } else if status.connecttool_status.can_start { "helper_ready" } else { "blocked_by_precheck" }.to_string(),
        message: if status.connecttool_status.helper_running {
            "ConnectTool helper 已运行。请在 helper 中选择 TCP 转发、创建房间并邀请好友。".to_string()
        } else if status.connecttool_status.can_start {
            "ConnectTool 兼容模式可启动。请先启动 helper，再在 helper 中创建 TCP 转发房间。".to_string()
        } else {
...
pub fn start_steam_p2p_guest(input: SteamP2pGuestRequest) -> Result<SteamP2pSessionStatus, String> {
    ...
    let invite = SteamP2pInvitePacket {
        ...
        app_id: input.app_id.or_else(|| status.connecttool_status.app_id.clone()).or_else(|| status.app_id.clone()).unwrap_or_default(),
        created_at: Utc::now().to_rfc3339(),
    };
    let session = SteamP2pSessionStatus {
        running: status.connecttool_status.helper_running,
...
pub fn stop_steam_p2p_session() -> Result<SteamP2pSessionStatus, String> {
    ...
    let session = SteamP2pSessionStatus {
        ...
        message: "Steam P2P 会话状态已清空；ConnectTool helper 如需关闭请使用停止 helper。".to_string(),
        invite: None,
        status,
    };
...
pub fn get_steam_p2p_session_status() -> Result<SteamP2pSessionStatus, String> {
    ...
    Ok(SteamP2pSessionStatus {
        ...
        message: "尚未启动 Steam Relay / P2P 会话。当前可使用 ConnectTool 兼容模式启动外部 helper。".to_string(),
        invite: None,
        status: get_steam_relay_status(),
    })
}

fn inspect_connecttool(input_dir: Option<&str>, steam: &(bool, Option<String>)) -> ConnectToolStatus {
    let directory = resolve_connecttool_dir(input_dir);
    let directory_exists = directory.as_ref().map(|path| path.is_dir()).unwrap_or(false);
    let mut file_statuses = Vec::new();
    let mut missing_files = Vec::new();

    let required = ["connecttool-qt.exe", "steam_api64.dll", "steamwebrtc64.dll", "steam_appid.txt"];
    ...
    if let Some(dir) = directory.as_ref() {
        ...
        for name in required {
            missing_files.push(name.to_string());
            file_statuses.push(ConnectToolFileStatus { name: name.to_string(), required: true, found: false, path: None, sha256: None });
        }
        file_statuses.push(ConnectToolFileStatus { name: "wintun.dll".to_string(), required: false, found: false, path: None, sha256: None });
    }

    let required_files_ok = directory_exists && missing_files.is_empty();
    let app_id_path = directory.as_ref().map(|dir| dir.join("steam_appid.txt")).filter(|path| path.is_file());
    let app_id = app_id_path.as_ref().and_then(|path| std::fs::read_to_string(path).ok()).map(|value| value.trim().to_string()).filter(|value| !value.is_empty());
    let wintun_available = file_statuses.iter().any(|item| item.name.eq_ignore_ascii_case("wintun.dll") && item.found);
    ...
    if !directory_exists {
        ...
        next_steps.push("在高级工具里填入 connecttool-qt 解压目录，或放到默认示例路径后重新检测。".to_string());
    }
    if directory_exists && !required_files_ok {
        diagnostics.push(format!("ConnectTool helper 缺少必需文件：{}。", missing_files.join("、")));
    ...
    if required_files_ok && !wintun_available {
        next_steps.push("TCP 转发可用；如需 TUN 组网，请确认 wintun.dll 存在并以管理员权限允许驱动/防火墙。".to_string());
    }
    if can_start && !running.0 {
        next_steps.push("可以启动 ConnectTool helper。房主或加入者仍需在 helper 中完成房间/Steam ID/端口配置。".to_string());
    }
    if running.0 {
        next_steps.push("helper 正在运行。请切到 helper 窗口完成 TCP 转发或 TUN 组网设置。".to_string());
    }
...
fn resolve_connecttool_dir(input_dir: Option<&str>) -> Option<PathBuf> {
    ...
    Some(PathBuf::from(DEFAULT_CONNECTTOOL_DIR))
}
...
fn detect_connecttool_process() -> (bool, Option<String>, Option<u32>) {
    #[cfg(target_os = "windows")]
    {
        ...
        if let Ok(output) = Command::new("tasklist").args(["/FI", "IMAGENAME eq connecttool-qt.exe"]).output() {
            let text = String::from_utf8_lossy(&output.stdout).to_lowercase();
