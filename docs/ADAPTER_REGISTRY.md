...
适配器管理页现在可以直接生成共享库提交说明。管理员认定并保存本地 custom adapter 后：

1. 在“适配器管理”表格中点击该游戏的“导出”。
2. 复制“导出结果”，保存为提交说明中提示的路径，例如：

```text
adapter-registry/games/example_game.json
```

3. 复制“共享库提交说明”里的 `index.json` entry，把它加入或更新到：

```text
adapter-registry/index.json
```

4. 确认 `sha256` 与提交说明一致。客户端同步 registry 时会用该 hash 校验远程 adapter。
5. 提交 Git 后，其他用户可以通过默认共享库地址同步到该 adapter。
