...
4. 仍有少量正式前端缺口需要保留记录，例如诊断目标选择器、部分页面真实列表正式受控化、方案库手动刷新语义等。
...
- Product Mode 会用最近选中游戏调用 `generateDiagnosticReportForGame(gameId)`。
- 没有选中游戏时回退全局诊断。

缺口：

- 用户不能在正式 UI 中明确选择“全局诊断 / 当前游戏诊断 / 指定游戏诊断”。

下一步建议：

- Product Mode 可继续补一个诊断目标提示。
- 正式 UI 应增加诊断目标选择器。
