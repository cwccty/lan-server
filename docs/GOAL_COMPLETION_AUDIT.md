...
### 缺口 1：正式受控 React 数据流尚未完全替换 DOM patcher
...
- 通用组网中心
- Terraria 向导
- 游戏扫描 / 推荐方案 / 方案库 / 高级连接工具的部分参考演示卡片

现状：

- 关键真实数据和按钮已通过 Product Mode 面板/拦截器接入。
- 为保持 `src/reference-ui` 一比一复刻，原参考 UI 仍有部分演示 state，真实能力由 `src/reference-adapter` 补齐。

风险：

- 当前风险已显著降低，但如果要发布为完全产品化前端，仍建议把 Product Mode patcher 迁移成正式受控 React 组件。

下一步建议：

- 继续保持 `src/reference-ui` 一比一。
- 准备正式受控 UI 重构方案，或继续补齐剩余导出/复制类能力。
### 已关闭：方案库同步详情真实展示
...
1. **复制/导出诊断报告正式入口**：诊断报告已有真实数据，仍可补复制/导出文本按钮。
2. **好友席位后端化评估**：当前 localStorage 能满足本地邀请包；如要多人房间/云同步，需要后端 room API。
3. **受控 React 重构评估**：当前保持一比一参考 UI，通过 Product Mode patcher 对接；如要“完全产品化”，下一阶段应评估替换 DOM patcher。
