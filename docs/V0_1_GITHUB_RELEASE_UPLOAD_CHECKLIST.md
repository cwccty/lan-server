...
| --- | --- |
| Release tag | `v0.1.0` |
| Release title | `联机助手 v0.1.0 早期公开测试版` |
| 主程序 | `src-tauri\target\release\lan-helper.exe` |
| 本地发布包 | `release-artifacts\v0.1.0\` |
| Windows ZIP 资产 | `release-artifacts\LanHelper-v0.1.0-windows-x64.zip` |
...
以下项目如果没有真实执行，必须在 Release 文案中继续写为“建议补测”，不能写成“已通过”：
...
4. Tag 填写：`v0.1.0`。
5. Title 填写：`联机助手 v0.1.0 早期公开测试版`。
6. Release body 优先复制 `docs/GITHUB_RELEASE_DRAFT.md`。
7. 在正文末尾追加：
   - 真实 EXE 启动烟测结果；
   - 仍需补测项目；
   - 用户反馈模板链接或内容。
8. 面向普通测试用户，优先上传 `release-artifacts\LanHelper-v0.1.0-windows-x64.zip`。
...
- v0.1 是早期公开测试版。
- 主线能力是 n2n、Terraria 房主侧、TCP/UDP/广播桥、高级工具、共享游戏方案库和诊断报告。
- 不是所有游戏都能一键联机。
...
当前 v0.1 可以作为小范围早期公开测试版发布；发布前必须保留“建议补测”边界，发布后应把反馈沉淀为游戏方案、诊断分类和推荐方案，而不是只临时修 UI 文案。
